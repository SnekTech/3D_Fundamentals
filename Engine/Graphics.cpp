...
void Graphics::DrawTriangle(const Vec2& v0, const Vec2& v1, const Vec2& v2, Color c)
{
	...
	else if (pv1->y == pv2->y) // natual flat bottom
	{
	...
	else // general triangle
	{
		...
		const float alphaSplit =
			(pv1->y - pv0->y) /
			(pv2->y - pv0->y);
		const Vec2 vi = *pv0 * (1 - alphaSplit) + *pv2 * alphaSplit;
...
{
	// using pointers so we can swap (for sorting)
...
void Graphics::DrawFlatBottomTriangle(const Vec2& v0, const Vec2& v1, const Vec2& v2, Color c)
{
	...
	{
		// calculate start and end points
		// add 0.5 to y because we're calculating based on pixel CENTER
		const float px0 = m0 * ((float)y + 0.5f - v0.y) + v0.x;
...
void Graphics::DrawFlatTopTriangleTex(const TexVertex& v0, const TexVertex& v1, const TexVertex& v2, const Surface& tex)
{
	...
	// calculate start and end scanlines
	const int yStart = (int)ceil((double)v0.pos.y - 0.5);
	const int yEnd = (int)ceil((double)v2.pos.y - 0.5); // the scanline After the last line drawn
	...
	for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)
	{
		...
		for (int x = xStart; x < xEnd; x++, itcLine += dtcLine)
		{
			...
			// and with fp(float point) err, tc.x(or y) can be > 1.0 (by a tiny amount)
		}
	}
}
