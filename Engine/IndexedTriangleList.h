...
public:
	...
	static IndexedTriangleList<T> Load(const std::string& filename)
	{
		...
		// to vertex position, laid out as xyzxyzxyz... etc.
		...
		// then for any face f, the first index of that face is [f * 3u]
		...
		for (size_t f = 0; f < mesh.num_face_vertices.size(); f++)
		{
			...
			for (size_t vn = 0; vn < 3u; vn++)
			{
				...
				tl.indices.emplace_back(size_t(idx.vertex_index));
			}

			// reserve winding if file marked as CCW
			if (isCCW)
			{
	...
	void AdjustToTrueCenter()
	{
		...
		struct VertexAccessor
		{
			...
			// (pointer is used to iterate over member of class here)
