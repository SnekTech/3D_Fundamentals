...
void Game::ComposeFrame()
{
	...
	// transform rom model space -> world(/view) space
	...
	for (size_t i = 0, end = triangles.indices.size() / 3; i < end; i++)
	{
		...
		triangles.cullFlags[i] = (v1 - v0) % (v2 - v0) * v0 > 0;
	}
	// transform to screen size (includes perspective transform)
	for (auto& v : triangles.vertices)
	{
		pst.Transform(v);
	}
	...
	for (size_t i = 0, end = triangles.indices.size() / 3; i < end; i++)
	{
		// skip triangls previously determined to be back-facing
